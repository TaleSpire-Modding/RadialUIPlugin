...
namespace RadialUI.Creature_Menu_Patches
{
    ...
    internal class AttacksSubMenuPatch
    {
        ...
        internal static void Postfix(MapMenu map, object obj, Creature ____selectedCreature)
        {
            ...
            foreach (var key in RadialUIPlugin._onSubmenuAttacks.Keys.Where(key => RadialUIPlugin._onSubmenuAttacks[key].Item2 == null || RadialUIPlugin._onSubmenuAttacks[key].Item2(miniId, targetId)))
            {
                map.AddItem(RadialUIPlugin._onSubmenuAttacks[key].Item1);
            }
        }
    }
}